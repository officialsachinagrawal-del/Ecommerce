...
function Home() {
  ...
  const { loading, products, error, productsCount } = useSelector(
    ...
    useEffect(() => {
        ...
        // Load more products on home so the initial view feels richer.
      ...
      <div className="bg-gray-900 py-16">
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center">
            <button onClick={() => navigate('/products')} className="bg-indigo-600 text-white rounded-md px-6 py-3 text-xl ">Shop Now</button>
          </div>
        </div>
      </div>
